...
<script setup>
    ...
    const getPage = ev => {
        const { target } = ev;
            let page = table.page;
            if (target.classList.contains('page-link') && !target.getAttribute('aria-label')) {
    ...
    const getLink = (id) => {
        return route('admin::image.edit', { image: id });
    };
    
    const confirmDelete = (id) => {
        const response = confirm('Delete?');
        if (response) {
            axios.delete(route('admin::image.destroy', {image: id}))
                    .then(() => {
                        doSearch(table.page, );
                    });
        }
